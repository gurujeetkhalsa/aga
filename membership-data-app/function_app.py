...
# Migration note: this app exposes the membership import and member lookup endpoints.
...
def lookup_members(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if agaid is None and not last_name_prefix and not first_name_prefix:
        return func.HttpResponse(
            "At least one of 'agaid', 'last_name', or 'first_name' is required.",
            status_code=400,
        )
    ...
    try:
        ...
        return func.HttpResponse("Query parameter 'limit' must be an integer.", status_code=400)

    try:
        offset = int(offset_text)
    except ValueError:
        return func.HttpResponse("Query parameter 'offset' must be an integer.", status_code=400)

    if limit < 1:
        return func.HttpResponse("Query parameter 'limit' must be greater than 0.", status_code=400)
    if offset < 0:
        return func.HttpResponse("Query parameter 'offset' must be 0 or greater.", status_code=400)
    ...
    try:
        ...
        payload = {
            ...
            "results": [{key: _json_safe_value(value) for key, value in row.items()} for row in rows],
        }
        return func.HttpResponse(
            json.dumps(payload),
            mimetype="application/json",
        ...
        logging.exception("LookupMembers SQL execution failed")
        return func.HttpResponse(f"Lookup failed: {exc}", status_code=500)
    except Exception as exc:
        logging.exception("LookupMembers failed")
        return func.HttpResponse(f"Lookup failed: {exc}", status_code=500)
