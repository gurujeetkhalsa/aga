...
import ratings_explorer_display_support as explorer
...
def _parse_rating_bands(req: func.HttpRequest) -> tuple[list[str] | None, func.HttpResponse | None]:
    ...
    if invalid:
        return None, func.HttpResponse("Query parameter 'rating_bands' is invalid.", status_code=400)
...
def ratings_explorer_players_startup(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if startup_payload:
            ...
            return _json_response(
                _with_debug(
                    {"results": results, "meta": startup_payload.get("meta") or {}},
        ...
        return _json_response(
            _with_debug(
                {"results": explorer.search_players(conn_str, None, None, None, None, None, None, "all", None, None, limit)},
                data_source="sql_live",
                elapsed_ms=round((perf_counter() - started) * 1000, 1),
            )
        )
...
def ratings_explorer_tournaments(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        states = _parse_csv_values(req, "states", "state")
        description = (req.params.get("description") or "").strip() or None
        date_from = (req.params.get("date_from") or "").strip() or None
        date_before = (req.params.get("date_before") or "").strip() or None
        if snapshot:
        ...
        return _json_response(
            _with_debug(search_payload, data_source="sql_live", elapsed_ms=round((perf_counter() - started) * 1000, 1))
        )
    except Exception as exc:
...
def ratings_explorer_filter_options(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if filter_options:
            return _json_response(
                _with_debug(filter_options, data_source="filter_options_snapshot", elapsed_ms=round((perf_counter() - started) * 1000, 1))
            )
        ...
        if snapshot:
            return _json_response(
                _with_debug(
                    explorer.build_filter_options_from_snapshot(snapshot),
                    data_source="main_snapshot_fallback",
                    elapsed_ms=round((perf_counter() - started) * 1000, 1),
                )
            )
        ...
        if error:
            return error
        return _json_response(
            _with_debug(explorer.build_filter_options(conn_str), data_source="sql_live", elapsed_ms=round((perf_counter() - started) * 1000, 1))
        )
    except Exception as exc:
...
def ratings_explorer_player(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        history_points = explorer.load_sql_rating_history(agaid)
        ...
        payload["data_source"] = "sql_live"
        return _json_response(_with_debug(payload, data_source="sql_live", elapsed_ms=round((perf_counter() - started) * 1000, 1)))
    except Exception as exc:
...
def ratings_explorer_game_sgf_viewer(req: func.HttpRequest) -> func.HttpResponse:
    ...
    return func.HttpResponse(page, status_code=200, headers=explorer.response_headers("text/html; charset=utf-8"))
...
def ratings_explorer_asset(req: func.HttpRequest) -> func.HttpResponse:
    ...
    return func.HttpResponse(body=payload, status_code=200, headers=explorer.response_headers(content_type))
...
def ratings_explorer_player_history_svg(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        svg = explorer.render_single_history_svg(
            agaid,
            explorer.load_sql_rating_history(agaid),
            explorer.load_member_name(conn_str, agaid),
        )
        return func.HttpResponse(svg, status_code=200, headers=explorer.response_headers("image/svg+xml; charset=utf-8"))
...
def ratings_explorer_snapshot_status(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if snapshot_meta:
        return _json_response(
            _with_debug(
                {"snapshot_available": True, "meta": snapshot_meta, "job_status": status},
    ...
    return _json_response(
        _with_debug(
            {"snapshot_available": False, "meta": None, "job_status": status},
            data_source="none",
            elapsed_ms=round((perf_counter() - started) * 1000, 1),
        )
    )
...
def ratings_explorer_nightly_snapshot(timer: func.TimerRequest) -> None:
    ...
    try:
        ...
        explorer.update_snapshot_status("failed", source="timer", detail="Nightly snapshot refresh failed.", error=str(exc))
        raise
...
def ratings_explorer_pending_snapshot_refresh(timer: func.TimerRequest) -> None:
    ...
    try:
        ...
        explorer.update_snapshot_status("failed", source="manual-timer", detail="Pending snapshot refresh failed.", error=str(exc))
        raise
