...
- Rated-game source keys use `rated_game_participation:<Game_ID>:<AGAID>` to prevent duplicate awards when the same game data is processed repeatedly.
- A BayRate production rerun writes `ratings.bayrate_reward_reconciliations`. Rated-game automation excludes those rerun tournament codes so changed or newly allocated game IDs are handled only through the persisted reconciliation report. Cascaded rating recalculations do not write this marker and do not affect rewards.
- `rewards.sp_apply_bayrate_reward_reconciliation` turns a reviewed reconciliation report into ledger adjustments. It is dry-run by default, permits one application per BayRate run, creates two-year lots for positive differences, consumes the earliest-expiring unexpired lots for negative differences, and blocks if a chapter lacks enough points. Runs with no differences are recorded as `no_changes` without posting transactions.
- The Chapter Rewards admin page lists persisted BayRate reconciliations, shows every chapter's old/new played-game, total-games, State Championship, and total points, and requires explicit confirmation before applying the differences. The application records the authorized operator and the resulting Chapter Rewards run.
...
- `rewards.tournament_awards` groups committed `ratings.tournaments` rows by host chapter plus `Reward_Event_Key`, totals rated, non-excluded games across all sections in the group, and posts only the positive top-up if a later section increases the formula result. BayRate reruns store the recalculated host and State Championship entitlements as reconciliation baselines, preventing those rerun differences from being awarded automatically. BayRate's `AGA - No hosting chapter` selection records Congress and other no-host events explicitly but earns no tournament-host or State Championship points; rated-game participation awards are processed independently as usual.
...
- If any tournament section in a grouped event is marked `Reward_Is_State_Championship`, `rewards.tournament_awards` posts a separate `200,000` point State Championship award once for that host chapter and grouped event. A rerun never posts it again; any old/new State Championship difference appears in BayRate's reconciliation report.
