...
class MemChapCsvImportTest(unittest.TestCase):
    ...
    def test_staged_event_sql_adapter_commits_query_rows(self):
        """Verify that row-returning staged procedures are committed."""
        fake_pyodbc = _FakePyodbc()
        original_pyodbc = mailapp.pyodbc
        try:
            mailapp.pyodbc = fake_pyodbc
            adapter = mailapp._ClubExpressStagedEventSqlAdapter("conn")

            rows = adapter.query_rows("EXEC [rewards].[sp_process_chapter_renewal_notices] @MessageId = ?", ("msg",))
        finally:
            mailapp.pyodbc = original_pyodbc

        self.assertEqual(rows, [{"Result": "posted"}])
        self.assertEqual(fake_pyodbc.connection.cursor_obj.executed, [("EXEC [rewards].[sp_process_chapter_renewal_notices] @MessageId = ?", ("msg",))])
